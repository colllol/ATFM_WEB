...
$testNewtonsoft = Resolve-TestDependency @('packages\Newtonsoft.Json.9.0.1\lib\net45\Newtonsoft.Json.dll', 'QLB.API\bin\Newtonsoft.Json.dll')
$testOracle = Resolve-TestDependency @('ODAC121024Xcopy_x64\odp.net4\odp.net\bin\4\Oracle.DataAccess.dll', 'QLB.API\bin\Oracle.DataAccess.dll')
...
internal static class AiNotificationUpsertTests
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            phase = "read backend ConnectDB configuration";
            XmlDocument config = new XmlDocument { XmlResolver = null };
            config.Load(args[1]);
            XmlElement entry = config.SelectSingleNode("/configuration/appSettings/add[@key='ConnectDB']") as XmlElement;
            if (entry == null || string.IsNullOrWhiteSpace(entry.GetAttribute("value")))
            ...
            RunLive(entry.GetAttribute("value"));
...
try {
    ...
    $testCompilerArgs = @(
        ...
        (Join-Path $testRepoRoot 'QLB.API\Data\System\AiNotificationService.cs'), $testHarnessPath
    )
    & $testCompiler @testCompilerArgs
    if ($LASTEXITCODE -ne 0) { throw 'AI notification upsert test compilation failed.' }
    if ($RunLive) {
        & $testExecutable '--live' (Join-Path $testRepoRoot 'QLB.API\Web.config')
    } else {
        & $testExecutable
    }
