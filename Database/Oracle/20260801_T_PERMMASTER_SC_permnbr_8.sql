...
PROMPT === 1. Increase T_PERMMASTER_SC.PERM NBR to 8 characters ===
...
BEGIN
    ...
    IF l_data_type <> 'VARCHAR2' THEN
        RAISE_APPLICATION_ERROR(
            -20001,
            'T_PERMMASTER_SC.PERM NBR must be VARCHAR2 before migration'
        );
    ELSIF l_char_length = 5 THEN
        EXECUTE IMMEDIATE
            'ALTER TABLE T_PERMMASTER_SC MODIFY (PERMNBR VARCHAR2(8 CHAR))';
...
PROMPT === 2. Patch PERM_PKG search/duplicate checks without changing legacy 5-digit behavior ===
...
BEGIN
    ...
    IF DBMS_LOB.INSTR(l_ddl, l_old_search, 1, 1) > 0 THEN
        ...
        l_ddl := l_new_ddl;
        DBMS_OUTPUT.PUT_LINE('Updated SearchExtentsion_Ext PERMNBR condition.');
    ELSIF DBMS_LOB.INSTR(l_ddl, l_new_search, 1, 1) = 0 THEN
        RAISE_APPLICATION_ERROR(
            -20003,
            'Cannot find old or new SearchExtentsion_Ext PERMNBR condition'
        );
    ELSE
        DBMS_OUTPUT.PUT_LINE('SearchExtentsion_Ext condition is already updated.');
    END IF;
    ...
    IF l_position > 0 THEN
        ...
        IF INSTR(l_context, 'from t_permmaster_sc') = 0 THEN
            RAISE_APPLICATION_ERROR(
                -20004,
                'First legacy validFlightNbr condition is not the SC branch'
            );
        END IF;

        l_new_ddl := replace_first(l_ddl, l_old_valid, l_new_valid);
        l_ddl := l_new_ddl;
        DBMS_OUTPUT.PUT_LINE('Updated validFlightNbr SC condition.');
    ELSIF DBMS_LOB.INSTR(l_ddl, l_new_valid, 1, 1) = 0 THEN
        RAISE_APPLICATION_ERROR(
            -20005,
            'Cannot find old or new validFlightNbr SC condition'
        );
    ELSE
        DBMS_OUTPUT.PUT_LINE('validFlightNbr SC condition is already updated.');
    END IF;
...
SELECT OBJECT_TYPE, OBJECT_NAME, STATUS
 ...
 WHERE OBJECT_NAME IN
       (
           ...
           'FLIGHT_DAYFLIGHT',
           'PERMISSION2TEXT',
           'CONVERTPERMSCTOSCHEDULE_CHANGE',
           'CONVERTPERMSCTOSCHEDULE_DELETE',
           'PERMMASTER_SC_TRIGGER_HIS'
       )
