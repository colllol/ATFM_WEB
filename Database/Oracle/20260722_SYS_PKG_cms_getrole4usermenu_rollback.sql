-- Roll back the SYS_PKG.CMS_GetRole4UserMenu fix deployed on 2026-07-22.
-- WARNING: this restores the previous behavior that returns all T_USERMENU rows.
...
prompt [1/3] Restore the original unfiltered procedure
declare
    l_source       clob;
...
begin
    ...
    if l_error_count > 0 then
        raise_application_error(-20305,
            'SYS_PKG restore failed; IX_TUM_USER_MENU was not dropped.');
    end if;
end;
/
...
begin
    ...
    if l_count > 0 then
        execute immediate 'drop index IX_TUM_USER_MENU';
    end if;
end;
/

select object_name, object_type, status
  from user_objects where object_name = 'SYS_PKG'
