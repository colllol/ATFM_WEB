...
- Đã cấu hình khóa do người dùng cung cấp tại APIAIKey trong QLB.API/Web.config local; service gửi X-API-Key từ backend. Không commit cấu hình chứa khóa. Endpoint 192.168.100.136:8000 đang timeout từ máy dev, cần xác nhận địa chỉ/kết nối và cấu hình tương ứng trên server API.
