-- Backup package body before optimizing GetInboxBySearchLogFile.
...
END GetInboxBySearch;
	...
	begin
	   --insert into t_log values ('SEARCHINBOX1',p_toDate,p_fromDate);commit;
	   OPEN p_out FOR
	WITH base AS
	(
	    SELECT
	...
	ORDER BY rn;
		...
		open p_out for
				with base as (
					...
					r.id,
					r.letternbr_pk,
					row_number() over (order by r.letternbr_pk desc) rn
				    from T_Receive_Logfile r
				    where r.letternbr_pk >= p_fromDate
				      and r.letternbr_pk <  p_toDate + 1
				      and (p_nbr is null or trim(p_nbr) is null or upper(trim(r.nbr)) = upper(trim(p_nbr)))
				      and (p_origin is null or trim(p_origin) is null or upper(r.from_pl) like '%' || upper(trim(p_origin)) || '%')
				      and (p_content is null or trim(p_content) is null or upper(r.content) like '%' || upper(trim(p_content)) || '%')
				)
				select nbr, type, id, letternbr_pk
				from base;

	
		open p_out for
	   select * from(
	      --  select IN_TIME NBR, 'DVKL' as Type, IN_ID ID, (IN_TIME) as LETTERNBR_PK from TBLRECEIVEMESSAGEINBOX
	       select NBR, 'DVKL' as Type, ID, (LETTERNBR_PK) as LETTERNBR_PK from T_Receive_Logfile
		    where
		    --to_date(LETTERNBR_PK,'dd-mm-yyyy')
		    LETTERNBR_PK
		    between to_date(p_fromDate,'dd-mm-yy')
		    and to_date((p_toDate+1),'dd-mm-yy')
