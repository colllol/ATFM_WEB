-- Toi uu transaction cua PERMDETAIL_SC_UPDATE sau khi da co trigger lich su.
-- Luu y: ConvertPermScToSchedule goi spRENDERSCHEDULEBY_Cus co COMMIT noi bo;
-- script nay khong thay doi hanh vi do de tranh anh huong cac chuc nang khac.
    ...
    BEGIN
        ...
        IF l_scope_pos = 0 OR l_start_pos = 0 OR l_end_pos = 0 THEN
            RAISE_APPLICATION_ERROR(
                -20001,
                'Cannot locate PERMDETAIL_SC_UPDATE patch markers.'
            );
        END IF;
...
BEGIN
    ...
    IF DBMS_LOB.INSTR(l_ddl, l_new_marker, 1, 1) > 0 THEN
        DBMS_OUTPUT.PUT_LINE('PERMDETAIL_SC_UPDATE is already optimized.');
        RETURN;
    END IF;

    -- Bo log tong quat va COMMIT truoc khi cap nhat du lieu chinh.
    REPLACE_BETWEEN(
        l_ddl,
        l_scope_marker,
    ...
    -- Khong commit ngay sau UPDATE; de viec xoa/tao lai lich bay tiep tuc xu ly.
    REPLACE_BETWEEN(
        l_ddl,
        l_scope_marker,
    ...
    -- Commit mot lan o cuoi procedure va rollback phan chua commit khi loi.
    REPLACE_BETWEEN(
        l_ddl,
        l_scope_marker,
        '      P_RETURN_CODE := nKq;',
        '  END PERMDETAIL_SC_UPDATE;',
          '      P_RETURN_CODE := nKq;' || CHR(10)
    ...
    DBMS_OUTPUT.PUT_LINE('Optimized PERM_PKG.PERMDETAIL_SC_UPDATE.');
END;
/
