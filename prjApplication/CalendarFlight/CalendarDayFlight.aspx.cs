...
{
    public partial class CalendarDayFlight : PageBaseCallBack
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                txtFLIGHTDATE1.Value = DateTime.Now.AddDays(1).ToString("dd/MM/yyyy");
                LoadData();
                lbnCreate.Visible = _Role.R_Add;
                btnRenderKhb.Visible = _Role.R_Pub;
                //btnRenderKhb_2.Visible = _Role.R_Pub;
        ...
        protected string IsStatusGet(string str)
        {
            string strReturn = "";
            if (str == "1")
                strReturn = prjComponents.Global.ApplicationPath + "/Images/check.png";
            if (str == "0")
                strReturn = prjComponents.Global.ApplicationPath + "/Images/cross-32.png";
            return strReturn;
        }
        ...
        {
            if(txtFLIGHTDATE1.Value=="")
            {
                this.AlertMessage("Please select flight date");
                return;
            }
            Int64 kq = Convert.ToInt64(new clsResuftAPI().GetValueApiExtension("FLIGHT_DAYFLIGHT", "AccessCalendar"
                , new { P_DATE = UltilFunc.ToDate(txtFLIGHTDATE1.Value, "dd/MM/yyyy"), P_USER = _user.UserName }));
            this.AlertMessage(kq==1?"Sussess":"Error!");
        }
    }
}