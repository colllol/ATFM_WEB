...
{
    public partial class CalendarDraft : PageBaseCallBack
    {
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.DataSource = new DayFlightsDAL().GetPageDraft(100000000, PhanTrang1.PageIndex, GetWhereConditionInGrid());
        ...
        void LoadData()
        {
            ...
            if (chkKhbTrungLap.Checked)
                t = new DayFlightsDAL().GetPageDraftTrungLap(PhanTrang1.PageSize, PhanTrang1.PageIndex, GetWhereConditionInGrid());
            else
                t = new DayFlightsDAL().GetPageDraft(PhanTrang1.PageSize, PhanTrang1.PageIndex, GetWhereConditionInGrid());
            if (t.Count > 0)
        ...
        {
            List<DayFlights> t = new DayFlightsDAL().GetPageDraft(PhanTrang1.PageSize, PhanTrang1.PageIndex, GetWhereConditionInGrid());
            PhanTrang1.TotalRecord = t.GetTotalRecord<DayFlights>();
            grdSource.DataSource = t;
            grdSource.DataBind();
        ...
        {
            DayFlights obj = new DayFlightsDAL().GetDraftById(id);
            var ser = new JavaScriptSerializer();
            ser.RegisterConverters(new JavaScriptConverter[] { new DateTimeConverter() });
            return ser.Serialize(obj);
        }
        ...
        private string btnCreateOnclick(string ThamSo)
        {
            ...
            bool kq = new DayFlightsDAL().InsertDraft(obj);
        ...
        private string btnUpdateOnclick(string ThamSo)
        {
            ...
            var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<DayFlights>(ThamSo, dateTimeConverter);
            obj.LASTUSER = _user.UserID.ToString();
            bool kq = new DayFlightsDAL().updateDraft(obj);
        ...
        {
            bool kq = new DayFlightsDAL().DeleteDraft(thamso);
        ...
        private string mbtnAddNewFlightDetailOnclick(string ThamSo)
        {
            try
            {
                var dateTimeConverter = new IsoDateTimeConverter { DateTimeFormat = "dd/MM/yyyy" };
                var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<DayFlights>(ThamSo, dateTimeConverter);
                var kq = new DayFlightsDAL().InsertDraft(obj);
        ...
        protected string rListHistoryFlightDetails(System.Data.DataTable dt, string keyId)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    //colum 4
                    kq += $"<td>{Convert.ToDateTime(dt.Rows[i]["LASTMODIFY"].ToString()).ToString("dd/MM/yyyy")}</td>";
                    //colum 5
