...
{
    public partial class ListReportingRoute : PageBaseCallBack
    {
        private const string _AliasSession = "ListReportingRoute";
        ...
        public override string GetCallbackResult()
        {
            ...
            switch (_arg[1])
            {
                case "GetAllReportingPoint":
                    kq = GetAllReportingPoint(_arg[0]);
                    break;
                case "chkCheck_Onchane":
                    kq = chkCheck_Onchane(ThamSo);
                    break;
                case "LoadDataGird":
                    kq = LoadDataGird();
                    break;               
        ...
        #endregion
        private string GetAllReportingPoint(string ThamSo)
        {
            List<ReportingPoint> obj = new ReportingPointDAL().GetAddReportingPoint(ThamSo);
            GridView1.DataSource = obj;
            GridView1.DataBind();
            var ax = this.RenderToHTML(GridView1);
            return ax;
        }
        ...
        private string chkCheck_Onchane(string[] thamso)
        {
            return new ReportingRouteDAL().SetPointIdIntoRouteID(thamso[0], thamso[1], thamso[2]) ? "true" : "false";
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteReportingRoute":
                    var kq = new ReportingRouteDAL().DeleteReportingRoute(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/REPORTING_ROUTE/EditReportingRoute.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ROUTE_ID=" + id.ToString());
                    break;
            }
        }
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.DataBind();
            this.CreateExcel(this.RenderToHTML(ax), "ListReportingRoute_" + DateTime.Now.ToFileTime() + ".xls");
        }
        ...
        {
            List<ReportingRoute> t = new ReportingRouteDAL().GetPageReportingRoute(PhanTrang1.PageSize, IsSearch ? 0 : PhanTrang1.PageIndex, txtSearch_UserName.Text.Trim());
            PhanTrang1.TotalRecord = t.GetTotalRecord<ReportingRoute>();
