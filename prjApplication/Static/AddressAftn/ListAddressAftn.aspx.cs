...
{ 
    public partial class ListAddressAftn : PageCoreAdmin
    {
        private const string _AliasSession = "ListAddressAftn";
        ...
        {
            List<prjInfo.AddressAftn> t = new AddressAftnDAL().GetPageObject(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<prjInfo.AddressAftn>();
        ...
        private void PutclsSearchDetail()
        {
            ...
            lisSearch.Add(new clsSearchDetail("ADDRESS", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        {
            Response.Redirect("~/Static/AddressAftn/EditAddressAftn.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + string.Format(" UPPER(ADDRESS) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()));
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new AddressAftnDAL().DeleteObject(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/AddressAftn/EditAddressAftn.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString());
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new AddressAftnDAL().GetPageGadExport(GetWhereCondition());
            else
                ax.DataSource = new AddressAftnDAL().GetAllObject();
            ax.DataBind();
