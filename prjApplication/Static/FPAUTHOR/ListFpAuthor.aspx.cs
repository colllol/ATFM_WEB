...
{   
    public partial class ListFpAuthor : PageCoreAdmin
    {
        private const string _AliasSession = "ListFpAuthor";
        ...
        {          
            List<FpAuthor> t = new FpAuthorDAL().GetPageFpauthor(CustomPaging1.PageSize, CustomPaging1.PageIndex ,CustomPaging1.ValueSearch);
            
            CustomPaging1.TotalsRecord = t.GetTotalRecord<FpAuthor>();
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + string.Format(" UPPER(AUTHOR_CODE) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()));
        ...
        //    lisSearch.Add(new clsSearchDetail("AUTHOR_CODE", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        {
            Response.Redirect("~/Static/FPAUTHOR/EditFpAuthor.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new FpAuthorDAL().DeleteObject(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/FPAUTHOR/EditFpAuthor.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString());
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new FpAuthorDAL().GetPageFpauthorExport(GetWhereCondition());
            else
                ax.DataSource = new FpAuthorDAL().GetAllObject();
            ...
            this.CreatePDF(html, "LISTFPAUTHOR_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.RenderControl(htw);
            string html = sw.ToString();
            this.CreateExcel(html, "LISTFPAUTHOR_" + DateTime.Now.ToFileTime() + ".xls");
        }
