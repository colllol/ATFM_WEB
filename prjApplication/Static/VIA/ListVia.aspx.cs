...
{
    public partial class ListVia : PageCoreAdmin
    {
        private const string _AliasSession = "ListVia";
        ...
        {
            List<Via> t = new ViaDAL().GetListPage(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<Via>();
        ...
        private void PutclsSearchDetail()
        {
            ...
            lisSearch.Add(new clsSearchDetail("SOHIEU", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + (string.Format(" UPPER(Oper) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()))+"OR"+ string.Format(" UPPER(VIA) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper())));
            return HttpUtility.UrlEncode(where.ToUpper());
        }

        protected void btnAddCountry_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/Static/VIA/EditVia.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new ViaDAL().DeleteVia(id);
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/VIA/EditVia.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString());
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new ViaDAL().GetListPageExport(GetWhereCondition());
            else
                ax.DataSource = new ViaDAL().GetListAll();
            ax.DataBind();
