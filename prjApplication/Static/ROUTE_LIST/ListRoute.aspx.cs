...
{
    public partial class ListRoute : PageCoreAdmin
    {
        private const string _AliasSession = "ListRoute";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                LoadData();
                btnAddRoute.Enabled = _Role.R_Add;
            }
        }
        ...
        {
            List<RouteList> t = new RouteListDAL().GetPageRouteList(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<RouteList>();
        ...
        private void PutclsSearchDetail()
        {
            ...
            lisSearch.Add(new clsSearchDetail("ROUTE_NAME", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + string.Format(" UPPER(ROUTE_NAME) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()));
            return HttpUtility.UrlEncode(where.ToUpper());
        }
        protected void btnAddRoute_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/Static/ROUTE_LIST/EditRoute.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new RouteListDAL().DeleteRouteList(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    obj = new RouteListDAL().GetOneRouteList(id);
                    Response.Redirect("~/Static/ROUTE_LIST/EditRoute.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString() + "&ROUTE_ID=" + obj.ROUTE_ID);
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new RouteListDAL().GetPageRouteListExport(GetWhereCondition());
            else
                ax.DataSource = new RouteListDAL().GetAllRouteList();
            ax.DataBind();
            ...
            this.CreatePDF(html, "LISTROUTE_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap1.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.DataBind();
            this.CreateExcel(this.RenderToHTML(ax), "LISTROUTE_" + DateTime.Now.ToFileTime() + ".xls");
        }
