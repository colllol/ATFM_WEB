...
{
    public partial class ListAero : PageCoreAdmin
    {
        private const string _AliasSession = "ListAero";
        ...
        {
            List<Aero> t = new AeroDAL().GetListPage(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<Aero>();
        ...
        private void PutclsSearchDetail()
        {
            ...
            lisSearch.Add(new clsSearchDetail("AE_NAME", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + (string.Format(" UPPER(AE_CODE) like '%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper())) + "OR" + string.Format(" UPPER(AE_IATA) like '%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper())));
            return HttpUtility.UrlEncode(where.ToUpper());
        }
        protected void btnAddObject_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/Static/Aero/EditAero.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            prjInfo.Aero _obj = null;
            switch (e.CommandName)
            {
                case "DeleteAero": 
                    var kq = new AeroDAL().DeleteAero(id);
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    _obj = new AeroDAL().GetAeroById(id.ToString());
                    Response.Redirect("~/Static/AERO/EditAero.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString() + "&AE_ID=" + _obj.AE_ID);
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if(!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
            {                
                ax.DataSource = new AeroDAL().GetListPageExport(GetWhereCondition());
            }                
            else
                ax.DataSource = new AeroDAL().GetListAll();
            ax.DataBind();
            ...
            this.CreatePDF(html, "LISTAERO_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap1.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.DataBind();
            this.CreateExcel(this.RenderToHTML(ax), "LISTAERO_" + DateTime.Now.ToFileTime() + ".xls");
        }
