...
{
    public partial class ListAFTN : PageCoreAdmin
    {
        private const string _AliasSession = "ListAFTN";
        ...
        {
            List<Aftn> t = new AftnDAL().GetPageObject(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<Aftn>();
        ...
        private void PutclsSearchDetail()
        {
            ...
            lisSearch.Add(new clsSearchDetail("GROUP_NAME", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        {
            Response.Redirect("~/Static/AFTN/EditAFTN.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + string.Format(" UPPER(GROUP_NAME) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()));
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new AftnDAL().DeleteObject(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    _obj = new AftnDAL().GetOneGam(id.ToString());                   
                    Response.Redirect("~/Static/AFTN/EditAFTN.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString() + "&G_A_M_ID=" + _obj.G_A_M_ID);
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new AftnDAL().GetPageGamExport(GetWhereCondition());
            else
                ax.DataSource = new AftnDAL().GetAllObject();
            ax.DataBind();
            ...
            this.CreatePDF(html, "LISTAFTN_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap1.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.DataBind();
            this.CreateExcel(this.RenderToHTML(ax), "LISTAFTN_" + DateTime.Now.ToFileTime() + ".xls");
        }
