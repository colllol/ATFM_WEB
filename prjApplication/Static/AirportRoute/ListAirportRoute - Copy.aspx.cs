...
{
    public partial class ListAirportRoute : PageCoreAdmin
    {
        private const string _AliasSession = "ListAirportRoute";
        ...
        public void LoadData()
        {
            ...
            List<prjInfo.AirportRoute> t = new AirportRouteDAL().GetPageAirportRoute(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<prjInfo.AirportRoute>();
        ...
        private void PutclsSearchDetail()
        {
            ...
            lisSearch.Add(new clsSearchDetail("ROUTE", "txtSearch_UserName", txtSearch_UserName.Text.Trim()));
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + string.Format(" UPPER(ROUTE) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()));
            return HttpUtility.UrlEncode(where.ToUpper());
        }
        protected void btnAddObject_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/Static/AirportRoute/EditAirportRoute.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteRoute":
                    var kq = new AirportRouteDAL().DeleteAirportRoute(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/AirportRoute/EditAirportRoute.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + e.CommandArgument.ToString());
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new AirportRouteDAL().GetPageAirportRouteExport(GetWhereCondition());
            else
                ax.DataSource = new AirportRouteDAL().GetAllAirportRoute();            
            ax.DataBind();
            ...
            this.CreatePDF(html, "LIST_AIRPORTROUTE_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap1.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.DataBind();
            this.CreateExcel(this.RenderToHTML(ax), "LIST_AIRPORTROUTE_" + DateTime.Now.ToFileTime() + ".xls");
        }        
