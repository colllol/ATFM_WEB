...
{
    public partial class ListFlyPurpose : PageCoreAdmin
    {
        private const string _AliasSession = "ListFlyPurpose";
        ...
        {
            List<FlyPurpose> t = new FlyPurposeDAL().GetPageFlyPurpose(CustomPaging1.PageSize,CustomPaging1.PageIndex, CustomPaging1.ValueSearch);

            CustomPaging1.TotalsRecord = t.GetTotalRecord<FlyPurpose>();
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + string.Format(" UPPER(PURPOSE_CODE) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper()));
            return HttpUtility.UrlEncode(where.ToUpper());
        }
        protected void btnAddObject_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/Static/FLY_PURPOSE/EditFlyPurpose.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new FlyPurposeDAL().DeleteObject(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/FLY_PURPOSE/EditFlyPurpose.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString());
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new FlyPurposeDAL().GetPageFlyPurposeExport(GetWhereCondition());
            else
                ax.DataSource = new FlyPurposeDAL().GetAllObject();
            ax.DataBind();
            ...
            this.CreatePDF(html, "LISTFLYPURPOSE_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap1.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.RenderControl(htw);
            string html = sw.ToString();
            this.CreateExcel(html, "LISTFLYPURPOSE_" + DateTime.Now.ToFileTime() + ".xls");
        }
