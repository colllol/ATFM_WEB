...
{
    public partial class ListCountry : PageCoreAdmin
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                LoadData();
                btnAddCountry.Enabled = _Role.R_Add;
            }
        }
        ...
        {
            List<Ctry> t = new CtryDAL().GetPageCountry(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<Ctry>();
        ...
        public string GetWhereCondition()
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                where += " AND " + (string.Format(" UPPER(CTR_CODE) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper())) + "OR" + string.Format(" UPPER(CTR_ENAME) like N'%{0}%'", UltilFunc.SqlFormatText(this.txtSearch_UserName.Text.Trim().ToUpper())));
        ...
        {
            Response.Redirect("~/Static/Country/EditCountry.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString());
        }
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new CtryDAL().DeleteCountry(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/Static/Country/EditCountry.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString());
                    break;
            }
        }
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(txtSearch_UserName.Text.Trim()))
                ax.DataSource = new CtryDAL().GetPageCountryExport(GetWhereCondition());            
            else
                ax.DataSource = new CtryDAL().GetAllCountry();
            ax.DataBind();
            ...
            this.CreatePDF(html, "LISTCOUNTRY_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/assets/css/bootstrap1.min.css"), System.Drawing.Printing.PaperKind.A4);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            ax.RenderControl(htw);
            string html = sw.ToString();
            this.CreateExcel(html,"LISTCOUNTRY_" + DateTime.Now.ToFileTime() + ".xls");
        }
