...
FROM (
    SELECT N.ID, N.TITLE, N.CONTENT, N.DATETIME, N.SOURCE_TYPE,
           ...
           SUM(CASE WHEN R.NOTIFICATION_ID IS NULL THEN 1 ELSE 0 END) OVER () UNREAD_COUNT,
    ...
    WHERE N.SOURCE_TYPE = 'AI'
      AND (:status = -1 OR CASE WHEN R.NOTIFICATION_ID IS NULL THEN 0 ELSE 1 END = :status)
)
