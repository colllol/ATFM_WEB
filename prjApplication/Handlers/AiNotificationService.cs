    ...
    internal static class AiNotificationService
    {
        ...
        internal static void QueueSynchronization()
        {
            ...
            try
            {
                HostingEnvironment.QueueBackgroundWorkItem(token =>
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        System.Diagnostics.Trace.TraceWarning("[AI notification sync] " + ex.GetType().Name);
                    }
            ...
            catch (Exception ex)
            {
                ...
                System.Diagnostics.Trace.TraceWarning("[AI notification queue] " + ex.GetType().Name);
            }
        }
...
AND AI_NOTIFICATION_PKG.CAN_RECEIVE(N.AI_RECIPIENT_ID, :P_USER)=1"))
        ...
        private static OracleConnection CreateConnection()
        {
            ...
            if (settings == null || string.IsNullOrWhiteSpace(settings.ConnectionString)) throw new ConfigurationErrorsException("Missing SlotsOracle.");
