...
namespace prjApplication.UploadMulti
{
    ...
    public class UploadAds1 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (Global.UseUpload_FTP == 1)
                {
                    ...
                    {
                        Bitmap logo = new Bitmap(_logo);
                        Image obj_InImage = null;
                        MemoryStream msroot = new MemoryStream(binData);
                        obj_InImage = Image.FromStream(msroot);
                        int positionX = 0, positionY = 0;
                        int phWidth = obj_InImage.Width;
                        int phHeight = obj_InImage.Height;
                        if (WaterMark == 1)
                        {
                            positionX = Convert.ToInt32(phWidth * 0.5) - Convert.ToInt32(logo.Width * 0.5);
                            positionY = Convert.ToInt32(phHeight * 0.5) - Convert.ToInt32(logo.Height * 0.5);
                        }
                        else if (WaterMark == 2)
                        {
                            positionX = spacevalue;
                            positionY = spacevalue;
                        }
                        else if (WaterMark == 3)
                        {
                            positionX = Convert.ToInt32(phWidth) - Convert.ToInt32(logo.Width) - spacevalue;
                            positionY = spacevalue;
                        }
                        else if (WaterMark == 4)
                        {
                            positionX = spacevalue;
                            positionY = Convert.ToInt32(phHeight) - Convert.ToInt32(logo.Height) - spacevalue;
                        }
                        else if (WaterMark == 5)
                        {
                            positionX = Convert.ToInt32(phWidth) - Convert.ToInt32(logo.Width) - spacevalue;
                        ...
                        if (WaterMark > 0 && WaterMark <= 5)
                        { imgOut = HPCImages.WatermarkImages(new Bitmap(obj_InImage), logo, positionX, positionY); }
                        else
                        {
                    ...
                    if (extenfile.ToLower() != "flv" && extenfile.ToLower() != "swf" && extenfile.ToLower() != "mp3" && extenfile.ToLower() != "mp4" && extenfile.ToLower() != "wmv" && extenfile.ToLower() != "doc" && extenfile.ToLower() != "docx" && extenfile.ToLower() != "xls" && extenfile.ToLower() != "rar" && extenfile.ToLower() != "txt" && extenfile.ToLower() != "pdf")
                    {
                        checkresize = true;
                    }
                    ...
                    if (IsImage(postedFile))
                    {
                        ...
                        _obj = SetItem(fileSave, postedFile.ContentLength, fileURL, extenfile, Convert.ToInt16(strUserID), Convert.ToInt16(vType), CATID, 0);
                ...
                else
                {
                    ...
                    if (_extenfile.ToLower() != ".flv" && _extenfile.ToLower() != ".swf" && _extenfile.ToLower() != ".mp3" && _extenfile.ToLower() != ".mp4" && _extenfile.ToLower() != ".wmv" && _extenfile.ToLower() != ".doc" && _extenfile.ToLower() != ".docx" && _extenfile.ToLower() != ".xls" && _extenfile.ToLower() != ".rar" && _extenfile.ToLower() != ".txt" && _extenfile.ToLower() != ".pdf")
                    {
                        ...
                        if (WaterMark > 0)
                        {
                            ...
                            HPCImageResize.SaveImage2Server(savepath, filename, "mark_" + _imagesEndWatermark, _imagesEndWatermark, _logo, Convert.ToInt32(prjComponents.Global.VNPResizeImagesContent), Convert.ToInt32(prjComponents.Global.VNPResizeImagesContent), WaterMark, spacevalue);
                        }
                        else
                            HPCImageResize.SaveImage2Server(savepath, filename, _imagesEndWatermark, Convert.ToInt32(prjComponents.Global.VNPResizeImagesContent), Convert.ToInt32(prjComponents.Global.VNPResizeImagesContent));
                    ...
                    //phan insert co so du lieu                   
                    _obj = SetItem(_imagesEndWatermark, postedFile.ContentLength, _urlSave, _extenfile, Convert.ToInt16(strUserID), Convert.ToInt16(vType), CATID, 0);
                    int _idReturn = _DAL.InsertT_ImageFiles(_obj);
                    //if (inStock != 0)
