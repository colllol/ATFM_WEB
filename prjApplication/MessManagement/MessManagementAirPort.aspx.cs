...
{
    public partial class MessManagementAirPort : PageBaseCallBack
    {
        ...
        {            
            DataTable dt = new DayFlightsDAL().GetTableWithPartNoAirPort(_date);
        ...
        {
            DataTable dt = new DayFlightsDAL().GetTableWithPartNoIDAirPort(txtBEGINDATE.Value, _partNo, messType);
            string kq="";
            if (dt.Rows.Count > 0)
            {
                kq = dt.Rows[0]["CONTENT"].ToString()+ "::" + dt.Rows[0]["ADDRESS"].ToString();
            }
            return kq;
        }
