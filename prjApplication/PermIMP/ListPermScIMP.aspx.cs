...
{
    public partial class ListPermScIMP : PageCoreAdmin
    {
        private const string _AliasSession = "ListPermScIMP";
        ...
        public void LoadData()
        {
            ...
            List<PermScIMP> t = new PermScImpDAL().GetPagePermScIMP(PhanTrang1.PageSize, PhanTrang1.PageIndex, where);
            if (t.Count == 0)
        ...
        protected void grdSource_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DeleteByID":
                    var kq = new PermScImpDAL().DeletePermScIMP(id.ToString());
                    if (kq) this.AlertMessage("Delete sussess!");
                    else this.AlertMessage("Delete error!");
                    LoadData();
                    break;
                case "Edit":
                    Response.Redirect("~/PermIMP/EditPermScImp.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString() + "&ID=" + id.ToString());
                    break;
            }
        }
        ...
        {
            var ax = Convert.ToInt64(new clsResuftAPI().GetValueApiExtension("PERM_IMP_PKG", "PermSc_BindToPerm", new { P_OPER = txtOper.Value.ToUpper() }).ToString());
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in rptSource.Items)
            {
                ...
                var txtFROMDATE = (System.Web.UI.HtmlControls.HtmlInputText)item.FindControl("txtFROMDATE");
                var txtTODATE = (System.Web.UI.HtmlControls.HtmlInputText)item.FindControl("txtTODATE");
                var txtDAILY = (System.Web.UI.HtmlControls.HtmlInputText)item.FindControl("txtDAILY");
                ...
                _obj.FROMDATE = txtFROMDATE.Value;
                _obj.TODATE = txtTODATE.Value;
                ...
                var ax = new PermScImpDAL().UpdatePermSCIMPAsync(_obj);
                if (ax) kq++;
            }
        ...
        {
            var ax = Convert.ToInt64(new clsResuftAPI().GetValueApiExtension("PERM_IMP_PKG", "DeleteImpByOper", new { P_OPER = txtOper.Value.ToUpper() }));
            if (ax == 1)
