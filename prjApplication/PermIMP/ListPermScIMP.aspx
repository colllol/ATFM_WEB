<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ATFM_New.Master" AutoEventWireup="true"
    CodeBehind="ListPermScIMP.aspx.cs" Inherits="prjApplication.PermIMP.ListPermScIMP"
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <table id="tblSource" class="table table-bordered">
        ...
        <thead>
            <tr>
                ...
                <td>
                    <input id="txtSearchREGISTRATION" runat="server" data-oldvalue='<%# Eval("REGISTRATION") %>' value='<%# Eval("REGISTRATION") %>' class="sInput"/></td>
                <td>
                    <input id="txtSearchFROMDATE" runat="server" data-oldvalue='<%# Eval("FROMDATE") %>' value='<%# Eval("FROMDATE") %>' class="sInput" /></td>
                <td>
                    <input id="txtSearchTODATE" runat="server" data-oldvalue='<%# Eval("TODATE") %>' value='<%# Eval("TODATE") %>' class="sInput"/></td>
                <td>
                    <input id="txtSearchDAILY" runat="server" data-oldvalue='<%# Eval("DAILY") %>' value='<%# Eval("DAILY") %>' class="sInput" />
                </td>
                <td>
            ...
            <asp:Repeater runat="server" ID="rptSource">
                <ItemTemplate>
                    <tr>
                        ...
                        <td>
                            <input id="txtREGISTRATION" class="wid_100px" runat="server" data-oldvalue='<%# Eval("REGISTRATION") %>' value='<%# Eval("REGISTRATION") %>' /></td>
                        <td>
                            <input id="txtFROMDATE" runat="server" class="wid_85px" data-oldvalue='<%# Eval("FROMDATE") %>' value='<%# Eval("FROMDATE") %>' />
                        </td>
                        <td>
                            <input id="txtTODATE" runat="server" class="wid_85px" data-oldvalue='<%# Eval("TODATE") %>' value='<%# Eval("TODATE") %>' />
                        </td>
                        <td>
                            <input id="txtDAILY" runat="server" class="wid_70px" data-oldvalue='<%# Eval("DAILY") %>' value='<%# Eval("DAILY") %>' />
                        </td>
                        <td>
 ...
 <td>
                            <input id="txtOper" class="wid_200px" runat="server" data-oldvalue='<%# Eval("OPER") %>' value='<%# Eval("OPER") %>' />
                        </td>
                        <td>
    ...
    <script type="text/javascript">
        function CheckOper() {
            //if ($('#ctl00_MainContent_txtOper').val().trim() == '') {alert('Oper no value!'); $('#ctl00_MainContent_txtOper').focus()return false;}
            //return confirm('Do you want render '+ $('#ctl00_MainContent_txtOper').val()+'?');
        }
    </script>
