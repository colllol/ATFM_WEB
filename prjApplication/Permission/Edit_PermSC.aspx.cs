...
{
    public partial class Edit_PermSC : PageBaseCallBack
    {
        ...
        {
            this.FillDropdownList(ddlAUTHOR_ID, new FpAuthorDAL().GetAllObject(), "AUTHOR_CODE", "AUTHOR_CODE");
            /*
            this.FillDropdownList(ddlAUTHOR_ID, new FpAuthorDAL().GetAllObject(), "AUTHOR_CODE", "AUTHOR_CODE");
            //this.FillDropdownList(ddlOPER_ID, new OperDAL().GetAllObject(), "OPER_ICAO", "OPER_ICAO");
            var s_Aero = new AeroDAL().GetListAll();
            var s_Purpo = new FlyPurposeDAL().GetAllObject();
            ...
            foreach (var item in s_Purpo)
            {
                ListItem li = new ListItem();
                li.Value = item.PURPOSE_CODE;
                li.Text = item.PURPOSE_CODE;
                ddlPURPOSE_ID.Items.Add(li);
            }
            ddlPURPOSE_ID.Items.Insert(0, new ListItem("--", ""));
            foreach (var item in s_Craft)
            {
                ListItem li = new ListItem();
                li.Text = item.MA + " - " + item.TAITRONG.ToString();
        ...
        private static void NormalizePermMasterSc(PermMasterSc obj)
        {
            if (obj == null)
        ...
        private string btnUpdateOnclick(string thamso)
        {
            try
            {
                ...
                return kq ? "Update success" : "Update error";
            }
            catch (Exception ex)
            {
        ...
        private string btnCreate_Details_Onclick(string ThamSo)
        {
            try
            {
                ...
                if (obj.PERM_ID == 0)
                    return "-1";
                if (obj.CRAFT_ID <= 0)
                    return "-2|Vui lòng chọn loại tàu bay (Craft) từ danh sách gợi ý.";
        ...
        protected void AsyncFileUpload1_UploadedComplete(object sender, AjaxControlToolkit.AsyncFileUploadEventArgs e)
        {
            ...
            Session.Add("fileUp", pathFile);
            Session.Add("fileurl", Global.ApplicationPath + @"/FileUploadPermission/" + DateTime.Today.Year + @"/" + DateTime.Today.Month + "//" + "SC//");
        }
