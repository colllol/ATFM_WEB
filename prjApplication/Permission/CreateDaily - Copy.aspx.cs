...
{
    public partial class CreateDaily : PageBaseCallBack
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    Session["MySearchDAYFLIGHT"] = null;
                    try
                    {
                        //var ax = Convert.ToInt64(Request["time"]); Session.SetValueSearch(_AliasSession, " 1=1 AND LETTERNBR_PK ='" + Request["time"].ToString()+"';");
                    }
                    catch { return; }
                ...
                {
                    //Session.SetValueSearch(_AliasSession, " 1=1 AND LETTERNBR_PK='" + Request["time"].ToString()+"';");
        ...
        protected void linkSearch_Click(object sender, EventArgs e)
        {
            ...
            Session["MySearchDAYFLIGHT"] = DataTableSearch();
        ...
        public DataTable DataTableSearch()
        {
            ...
            dt.Columns.Add(new DataColumn("LETTERNBR_PK", typeof(string)));
            dt.Columns.Add(new DataColumn("NBR", typeof(string)));
            dt.Columns.Add(new DataColumn("OPER_ID", typeof(string)));
            dt.Columns.Add(new DataColumn("PLAN_STATUS", typeof(string)));
            dt.Columns.Add(new DataColumn("REMARK", typeof(string)));
            dt.Columns.Add(new DataColumn("CODE", typeof(string)));
            dt.Columns.Add(new DataColumn("DOF", typeof(string)));
            foreach (GridViewRow row in grdSource.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    TextBox txtColum24 = (TextBox)row.FindControl("txtColum24");
                    TextBox txtColum25 = (TextBox)row.FindControl("txtColum25");
                    TextBox txtColum26 = (TextBox)row.FindControl("txtColum26");
                    ...
                    dr[24] = txtColum24.Text.ToString().Trim();
                    dr[25] = txtColum25.Text.ToString().Trim();
                    dr[26] = txtColum26.Text.ToString().Trim();
                    dt.Rows.Add(dr);
                }
                break;
            }
            ...
            foreach (GridViewRow row in grdSource.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (!String.IsNullOrEmpty(txtColum19.Text.Trim()))
                        where += " AND STATUS = " + Convert.ToInt32(txtColum19.Text.Trim());
                    if (!String.IsNullOrEmpty(txtColum20.Text.Trim()))
                        where += " AND LETTERNBR_PK  >=" + whereDateHelper(txtColum09.Text.Trim()) + "";
                    if (!String.IsNullOrEmpty(txtColum21.Text.Trim()))
                        where += " AND " + string.Format(" UPPER(NBR) like N'%{0}%'", UltilFunc.SqlFormatText(txtColum21.Text.Trim()));
                    if (!String.IsNullOrEmpty(txtColum22.Text.Trim()))
                        where += " AND " + string.Format(" UPPER(OPER_ID) like N'%{0}%'", UltilFunc.SqlFormatText(txtColum22.Text.Trim()));
                    if (!String.IsNullOrEmpty(txtColum23.Text.Trim()))
                        where += " AND PLAN_STATUS = " + Convert.ToInt32(txtColum23.Text.Trim());
                    if (!String.IsNullOrEmpty(txtColum24.Text.Trim()))
                        where += " AND " + string.Format(" UPPER(REMARK) like N'%{0}%'", UltilFunc.SqlFormatText(txtColum24.Text.Trim()));
                    if (!String.IsNullOrEmpty(txtColum25.Text.Trim()))
                        where += " AND " + string.Format(" UPPER(CODE) like N'%{0}%'", UltilFunc.SqlFormatText(txtColum25.Text.Trim()));
                    if (!String.IsNullOrEmpty(txtColum26.Text.Trim()))
                        where += " AND DOF  >=" + whereDateHelper(txtColum26.Text.Trim()) + "";
                }
                break;
            }
        ...
        public DataView BindGridData(DataTable _dt)
        {
            ...
            try
            {
                ...
                if (_dt != null)
                {
                    if (_dt.Rows.Count > 0)
                    {
                        if (Session["MySearchDAYFLIGHT"] != null)
                        {
                            DataTable dtseach = (DataTable)Session["MySearchDAYFLIGHT"];
                            dt.Rows.Add(dtseach.Rows[0]["PERMNBR"].ToString(), "0", dtseach.Rows[0]["PERMTYPE"].ToString(), dtseach.Rows[0]["FLIGHT_TYPE"].ToString(), dtseach.Rows[0]["PURPOSE"].ToString(), dtseach.Rows[0]["CRAFT_ID"].ToString(), dtseach.Rows[0]["MTOW"].ToString(), dtseach.Rows[0]["VALIDHOURS"].ToString(),
                                ...
                                , dtseach.Rows[0]["ETD"].ToString(), dtseach.Rows[0]["ETA"].ToString(), dtseach.Rows[0]["ATD"].ToString(), dtseach.Rows[0]["ATA"].ToString(), dtseach.Rows[0]["VIA"].ToString(), dtseach.Rows[0]["STATUS"].ToString()
                                , dtseach.Rows[0]["LETTERNBR_PK"].ToString(), dtseach.Rows[0]["NBR"].ToString(), dtseach.Rows[0]["OPER_ID"].ToString(), dtseach.Rows[0]["PLAN_STATUS"].ToString(), dtseach.Rows[0]["REMARK"].ToString(), dtseach.Rows[0]["CODE"].ToString(), dtseach.Rows[0]["DOF"].ToString());
                        }
                        else
                            dt.Rows.Add("", "0", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
                        for (int i = 0; i < _dt.Rows.Count; i++)
                        {
                            ...
                            dr[19] = _dt.Rows[i]["STATUS"].ToString();

                            if (_dt.Rows[i]["LETTERNBR_PK"] != Convert.DBNull)
                            {
                                DateTime _dtleter = Convert.ToDateTime(_dt.Rows[i]["LETTERNBR_PK"].ToString());
                            ...
                            dr[21] = _dt.Rows[i]["NBR"].ToString();
                            dr[22] = _dt.Rows[i]["OPER_ID"].ToString();
                            dr[23] = _dt.Rows[i]["PLAN_STATUS"].ToString();
                            dr[24] = _dt.Rows[i]["REMARK"].ToString();
                            dr[25] = _dt.Rows[i]["CODE"].ToString();
                            if (_dt.Rows[i]["DOF"] != Convert.DBNull)
                            {
                                DateTime _dtdof = Convert.ToDateTime(_dt.Rows[i]["DOF"].ToString());
                                if (_dtdof != DateTime.MinValue)
                                    dr[26] = _dtdof.ToString("dd/MM/yyyy");
                                else
                                    dr[26] = "";
                            }
                            else
        ...
        void LoadData()
        {
            List<DayFlights> t = new DayFlightsDAL().GetPageObject(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            CustomPaging1.TotalsRecord = t.GetTotalRecord<DayFlights>();
            DataTable dt = new DayFlightsDAL().GetTableObject(CustomPaging1.PageSize, CustomPaging1.PageIndex, CustomPaging1.ValueSearch);
            //Add by Nvthai
        ...
        private void LoadMultiDropdownList()
        {
            ...
            this.FillDropdownList<Oper>(mOPER_ID, new OperDAL().GetAllObject(), "OPER_NAME", "ID");
        }
        ...
        protected void grdSource_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowIndex == 0 && e.Row.RowType != DataControlRowType.Header)
            {
                ...
                TextBox txtColum24 = (TextBox)e.Row.FindControl("txtColum24");
                txtColum24.Visible = true;
                TextBox txtColum25 = (TextBox)e.Row.FindControl("txtColum25");
                txtColum25.Visible = true;
                TextBox txtColum26 = (TextBox)e.Row.FindControl("txtColum26");
                txtColum26.Visible = true;
                ...
                HtmlControl htmlcol24 = (HtmlControl)e.Row.FindControl("col24");
                htmlcol24.Attributes.Add("style", "display:none;");

                HtmlControl htmlcol25 = (HtmlControl)e.Row.FindControl("col25");
                htmlcol25.Attributes.Add("style", "display:none;");

                HtmlControl htmlcol26 = (HtmlControl)e.Row.FindControl("col26");
                htmlcol26.Attributes.Add("style", "display:none;");
