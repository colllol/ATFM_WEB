<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ATFM_New.Master" AutoEventWireup="true" CodeBehind="EditPermSC.aspx.cs" Inherits="prjApplication.Permission.EditPermSC" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div id="divPermDetail">
        ...
        <button type="button" id="btnGenList" class="btn btn-sm btn-primary"  onclick="btnUpdatebByID_GenBack()">Gen Flights</button>
        ...
        <button type="button" id="btnClearSearch" class="btn btn-sm btn-primary" onclick="lnkClear_Click()">Clear search</button>
         <a style="width:90px;" href="<%= Page.ResolveUrl("~/Permission/ListPermissionSC.aspx?Menu_ID=" + Page.Request["Menu_ID"].ToString()) %>"
        ...
        <table id="tblSource" class="">
            <thead>
                ...
                <tr>
                    <th></th>
                    <th><input id="txtFLIGHTNBR" runat="server" type="text" class="wid_80px" data-control="btnUpdate" maxlength="20"
                            data-minlenght="1" /></th>
                    <th><input id="txtREGISTRATION"  runat="server" maxlength="20" data-control="btnUpdate" data-minlenght="1"
                            type="text" class="wid_80px" /></th>
                    <th style="color: black!important;">
                        <input id="ddlFROM_AIRP" type="text" style="color: black;" class="wid_50px" data-autocomplete="AERO" />
                        
                    </th>
                    <th style="color: black!important;">
                        <input runat="server" id="ddlTO_AIRP" type="text" style="color: black;" class="wid_50px" data-autocomplete="AERO" />
                    ...
                    <th>
                        <input id="txtMTOW" runat="server" class="wid_50px"  disabled="disabled" maxlength="100" type="text" />
                    </th>
                    <th>
    ...
    <script>
        ...
        var ddlFLIGHTTYPE = document.getElementById('ddlFLIGHTTYPE');
    ...
    <script>
        ...
        function checkPermNumber() {
            ...
            var $request = $.ajax({
                ...
                data: JSON.stringify({ P_FLIGHT_TYPE: 'SC', P_FLIGHTNBR: $('#txtPERMNBR').val().toUpperCase(), P_PERMTYPE : $('#ddlPERMTYPE').val().toUpperCase() }),
                complete: function () {
        ...
        function LoadDataAjax(){
            ...
            $.ajax({
                //async: false,
                method: "PUT",
                url: urlApi + "api/PermDetailSc/GetBySearch_New",                
                data: GetObjectInfo(),
                beforeSend: function(){
                    $('#tblSource tbody tr').remove();
                ...
                $.each(data.ListValue, function(a,b){
                    ...
                    if(b.PURPOSE_NAME=='0')
                    {
                        ...
                        + "<td><input type='text' data-control='_updateAll' data-minlenght='4' maxlength='9' class='sInput' id='txtFLIGHTNBR"+a+"' data-oldValue='"+returnEmpty(b.FLIGHTNBR)+"' value='"+returnEmpty(b.FLIGHTNBR)+"' onchange='checkIsUpdate(this)' /></td>"
                        + "<td><input type='text' data-control='_updateAll' class='sInput' id='txtREGISTRATION"+a+"' data-oldValue='"+returnEmpty(b.REGISTRATION)+"' value='"+returnEmpty(b.REGISTRATION)+"' onchange='checkIsUpdate(this)' /></td>"
                        + "<td><input type='text' data-control='_updateAll' data-minlenght='2'  class='sInput' id='txtFROM_AIRP"+a+"' data-oldValue='"+b.FROM_AIRP+"' data-autocomplete=\'AERO\' value='"+b.FROM_AIRP+"' onchange='checkIsUpdate(this)' /></td>"
                        + "<td><input type='text' data-control='_updateAll' data-minlenght='2' class='sInput' id='txtTO_AIRP"+a+"' data-oldValue='"+b.TO_AIRP+"' data-autocomplete=\'AERO\' value='"+b.TO_AIRP+"' onchange='checkIsUpdate(this)'/></td>" 
                        ...
                        + "<td><input type='text' data-control='_updateAll' title='"+returnEmpty(b.REMARK)+"' class='sInput' id='txtREMARK"+a+"' data-oldValue='"+returnEmpty(b.REMARK)+"' value='"+returnEmpty(b.REMARK)+"' onchange='checkIsUpdate(this)'/></td>"
                        + "<td>" +  b.TAITRONG + "</td>"
                        + "<td>" + b.LASTUSER + "</td>"
                        + "<td><input type='text' data-control='_updateAll' class='sInput' id='txtSL" + a + "' value='" + returnEmpty(b.PURPOSE_NAME) + "'/></td>"
        ...
        function btnUpdatebByID_GenBack(){
            var cf = confirm("Do you want gen flights to schedule ?");
            if (cf) {
                var url = urlApi + "api/ApiExtension/ExcuteReturnInt?packageName=A_TEST_SEARCH&storeName=GenFlightsSC_ByID";
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        //preloadImgAfterButton('btnGenList', 'loadingAccess');
                    }
                }).always(function (data) {
        ...
        function btnGenMessage(){
            var cf = confirm("Do you want gen message flights cancel  ?");
            if (cf) {
                var url = urlApi + "api/ApiExtension/ExcuteReturnInt?packageName=A_TEST_SEARCH&storeName=PermToCanCel";
        ...
        function btnGenMessageText(){           
            ...
            $.ajax({
                ...
                method: "PUT",
                url: url,
                data: JSON.stringify({ TYPE_ID: 1, PERMID: $('#perm_id').html(), DATE_FLY: dateFormat(new Date().setDate(new Date().getDate() + 1), 'dd-mm-yyyy').replace(/^(\d{2})\-(\d{2})\-(\d{4})$/, '$3/$2/$1') }),
            }).always(function (data) {
        ...
        function btnUpdate_GenBack(){
            var result = confirm("Do you want gen flights to schedule ?");
            if (result) {
                if(!checkValidCustomMinlenght('_updateAll')){
                    return;
                }
                ...
                var c=0;
                $.each($lis, function(a,b){
                    var _sl =parseInt($($(b).find('[id^="txtSL"]')[0]).val());
                    if (_sl == 0) {                        
                        var _obj={};
                        _obj['PERM_ID'] = $('#perm_id').html();
                        _obj['ETA'] = $($(b).find('[id^="txtETA"]')[0]).val();
                        ...
                        var $request = $.ajax({
                            async: false,
                            method: "PUT",
                            url: urlApi + "api/PermDetailSc/UpdatePermDetailSc_GenBack",                
                            data: _obj,                    
                        }).always(function( data ) {
                            if(data.Value==null)c++;
                        });
        ...
        window.onkeydown = function(e) {
            ...
            if (charCode == 38) {
                try {
                    ...
                    //$($($(e).closest('tr').prev()).find('td')[$(e).closest('td').index()]).find('input')[0].id
            ...
            if (charCode == 118) {                
                var _tr ="<tr id='_"+c+"' data-isInsert='true' onmouseover=\"rowId=$(this).prop('id');\" onmouseout='rowId=0;'>"
                   +  "<td id='b_"+c+"'>" +  (c-1) + "</td>"
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='4' maxlength='8' class='sInput' id='txtFLIGHTNBR"+c+"' value='" + $('#ddlOPER_ID').val() + "' onchange='checkIsUpdate(this)' /></td>"
                   + "<td><input type='text' data-control='_updateAll' class='sInput' id='txtREGISTRATION"+c+"' value='' onchange='checkIsUpdate(this)' /></td>"
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='2' class='sInput' id='txtFROM_AIRP"+c+"' onfocusin='binAutocomplete(this,\"AERO\")' value='' onchange='checkIsUpdate(this)' /></td>"
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='2' class='sInput' id='txtTO_AIRP"+c+"' onfocusin='binAutocomplete(this,\"AERO\")' value='' onchange='checkIsUpdate(this)'/></td>" 
                   + "<td><input type='text' data-control='_updateAll' maxlength='6' class='sInput' id='txtETD"+c+"'  value='' onchange='checkIsUpdate(this)'/></td>"   
                   + "<td><input type='text' data-control='_updateAll' maxlength='6' class='sInput' id='txtETA"+c+"'  value='' onchange='checkIsUpdate(this)'/></td>"  
                   ...
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='1' class='sInput' id='txtCRAFT_NAME"+c+"' data-craftid='0' onfocusin='binAutocomplete(this,\"CRAFT\")' value='' onchange='checkIsUpdate(this)'/></td>"
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='1' data-CheckDate='true' class='sInput' id='txtBEGINDATE"+c+"' value='' onchange='checkIsUpdate(this)' /></td>"
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='1' data-CheckDate='true' class='sInput' id='txtENDDATE"+c+"'value='' onchange='checkIsUpdate(this)'/></td>"
                   + "<td><input type='text' data-control='_updateAll' data-minlenght='1' class='sInput' id='txtPURPOSE_ID"+c+"' onfocusin='binAutocomplete(this,\"PURPOSE\")' onchange='checkIsUpdate(this)'/></td>"
                   + "<td><input type='text' data-control='_updateAll' class='sInput' id='txtVIA"+c+"' onchange='checkIsUpdate(this)'/></td>"
                   + "<td><input type='text' data-control='_updateAll' class='sInput' id='txtREMARK"+c+"' onchange='checkIsUpdate(this)'/></td>"
                   + "<td></td>"
                   + "<td></td>"
    ...
    <script>
        ...
        function btnPrint_OnClick() {
            var content = $('#txtPERMCONTENT').val();
