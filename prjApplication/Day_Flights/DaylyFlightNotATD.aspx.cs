...
{
    public partial class DaylyFlightNotATD : PageBaseCallBack
    {
        ...
        private string btnSearch_Onclick(string thamso)
        {
            ...
            switch (obj.OptionDate)
            {
                case "0":
                    ...
                    dt = new DayFlightsDAL().GetFlightAtdNotNull(obj);
                    break;
                case "-1":
        ...
        {
            string c0 = "", c1 = "", c2 = "", c3 = "", c4 = "", c5 = "", c6 = "", c7 = "", c8 = "", c9 = "", c10 = "", c11 = "", c12 = "", c13 = "", c14 = "", c15 = "", c16 = "", c17 = "", c18 = "", c19 = "", kq = "";
            string c4B = "", c5B = "";
            string mes = "";
            if (dt == null) return "";
            string _record = "0";
            foreach (DataRow r in dt.Rows)
            {
                ...
                c19 = $"<td style=\"background-color:white;\"><input type=\"checkbox\" id='chk_" + r["ID"] + "' /></td>";
                c2 = $"<td onmouseout=\"hideddrivetip();\" onmouseover=\"ddrivetip('{c2_Tip(r)}', 'aquamarine', 1000);\">"
                   + $"<span>{cellHasSpan(r["PERMNBR"].ToString().ToUpper())}</span></td>";
                ...
                c4B = $"<td>{cellHasSpan(r["ETD"])}</td>";

                c5 = $"<td class=\"{tdColorClassChange("TO_AIRP", r["CHANGEVALUE"].ToString())}\">"
                    + $"{cellHasSpan(r["TO_AIRP"])}</td>";

                c5B = $"<td>{cellHasSpan(r["ETA"])}</td>";

                c6 = $"<td class=\"{tdColorClassChange("EOBT", r["CHANGEVALUE"].ToString())} {(!CompareHour(r["ETD"], r["EOBT"]) ? ColorHelper.GetColorCode(clsColor.cssTextBaySom) : "")}\">{cellHasSpan(r["EOBT"].ToString())}</td>";


                c7 = $"<td>{r["ATD"]}</td>";

                c8 = $"<td>{cellHasSpan(r["ATA"].ToString())}</td>";
                ...
                /* c6, c10 chua bind */
                kq += c19 + c1 + c2 + c11 + c3 + c4 + c5 + c4B + c6 + c5B + c7 + c8 + c16 + c17 + c9 + c10 + c12 + c14 + c18;
                kq += "</tr>";
            }
            kq = System.Text.RegularExpressions.Regex.Replace(kq, @"\r\n?|\n", "</br>");
            return kq + "&&&&" + _record;
        ...
        {
            return RenderGrdSource_First(new DayFlightsDAL().GetFlightAtdNotNull(new clsDaylyFlightSearch() { KHUNGGIO1 = "0000", KHUNGGIO2 = "2359", RowStart = 1, RowFinish = 20, PERMTYPE = "", FLIGHTDATE = DateTime.Today }));
        }
        /// <summary>
        /// 
        /// </summary>
