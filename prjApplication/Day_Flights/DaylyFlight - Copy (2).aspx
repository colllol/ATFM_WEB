...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
		...
		function LoadDataGrid_ExportFinish() {
            var _obj = {P_USER:'<%= _user.UserName%>',P_DATE: $('#ddlDateFlight option:selected').text() == '' ? new Date().format('dd/MM/yyyy') : $('#ddlDateFlight option:selected').text().replace(/\//gi, '-').replace(/^(\d{2})\-(\d{2})\-(\d{4})$/, '$1/$2/$3') }
