<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ATFMNoneMenu.Master" AutoEventWireup="true" CodeBehind="DaylyFlightCancel.aspx.cs" Inherits="prjApplication.Day_Flights.DaylyFlightCancel" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="row">
        <div class="divHeader">
            <div id="infoTotal">
                <span>EXPORT FINISHED FLIGHT</span>
            </div>
            <div id="topBar1">
            ...
            <div id="topBar3">
                ...
                <select id="ddlExport"> 
                    <option value="0" selected="selected">Not Moved</option>                   
                ...
                <button id="btnUpdate" type="button"
                    data-dismiss="modal" class="btn btn-sm btn-primary" onclick="btnDeleteByChecked_Onclick()">
                    <i class="fa"></i>
                    Update
                </button>
                <button id="btnExportFished" type="button"
                    data-dismiss="modal" class="btn btn-sm btn-primary" onclick="LoadDataGrid_Finished()">
                    <i class="fa"></i>
                    Export Finished
                </button>
                <span id="totql"></span>
                <div id="topbarNotificon">
    ...
    <script>
        ...
        + "</option>"
                    + "<option value='0'>" + dateFormat(new Date(), 'dd/mm/yyyy')
    ...
    'viewPopupInfoExtension');
        ...
        function btnDeleteByChecked_Onclick() {
            ...
            var cf = confirm('Do you want change ?');
            if (cf) {
                $('#grdSource tbody td input[type="checkbox"]:checked').each(function (a, b) {
                    var id = $(b).prop('id').split('_')[1];                   
                    var url = urlApi + "api/ApiExtension/ExcuteReturnInt?packageName=FLIGHT_DAYFLIGHT&storeName=updateFlightBy_LetterType_CNL";
                    $.ajax({
                        async: false,
                        method: "PUT",
                        url: url,
                        data: JSON.stringify({ P_ID: id,P_LETTER_TYPE: $('#ddlLetter_Type').val().toUpperCase()}),
                    }).always(function (data) {
                        if (data.ListValue == null || data.ListValue == -1) {                            
                        } 
                    })
