			...
			.on('updateHelperPosition.xdsoft', function () {
			    ...
			    timerUpdate = setTimeout(function () {
			        ...
			        $dropdown.css($.extend(true, {
			            ...
			            width: options.dropdownWidth == '100%' ? ($input[0].offsetWidth < 50 ? "120" : $input[0].offsetWidth) : options.dropdownWidth
...
var urlApi = 'http://localhost:50591/ATFM_API/';
...
$("<link/>", {
    rel: "stylesheet",
    type: "text/css",
    href: urlApp + "/Style/assets/css/autocomplete.css"
}).appendTo("head");
$.ajax({
    async: false,
    method: "GET",
    url: urlApi + "api/Oper/GetAllOper",
}).done(function (data) {
    listOper = [];
    $.each(data.ListValue, function (a, b) {
        var li = { icao: b.OPER_ICAO + ' - ' + b.OPER_IATA, name: b.OPER_ICAO + ' - '+ b.OPER_IATA + ' - ' + b.OPER_NAME};
        listOper.push(li);
    });
});
$.ajax({
    async: false,
    method: "GET",
    url: urlApi + "api/FlyPurpose/GetAllFlyPurpose",
}).done(function (data) {
    var li = '';
    $.each(data.ListValue, function (a, b) {
        li += b.PURPOSE_CODE + ',';
    });
    listPurpose = li.split(',');
});
$.ajax({
    async: false,
    //method: "PUT",
    method: "GET",
    //url: urlApi + "api/ApiExtension/ExcuteTable?packageName=DIC_PKG&storeName=AERO_GET_complete",
    url: urlApi + "api/Aero/GetAllAero",//,
    //data: {},
}).done(function (data) {
    listAero = [];
    $.each(data.ListValue, function (a, b) {
        var li = { AE_CODE: b.AE_CODE, AE_REM: b.AE_IATA };
        listAero.push(li);
    });
    //listAero = li.split(',');
});
$.ajax({
    async: false,
    method: "GET",
    url: urlApi + "api/CraftType/GetAllCraftType",
}).done(function (data) {
    listCraft = [];
    $.each(data.ListValue, function (a, b) {
        var li = { craftid:   b.CRAFT_ID  , name: b.MA };
        listCraft.push(li);
    });
});
...
function CheckListOper(ele) {
    if (listOper.findIndex(item=>item.icao.split('-')[0].trim() == $(ele).val().toUpperCase().split('-')[0].trim()) == -1) {
        $(ele).val('');
        $(ele).css('border-color: red;');
    } else {
        $(ele).css('border-color: \'\'');
        $(ele).val($(ele).val().toUpperCase().split('-')[0].trim());
        //$(ele).attr('data-craftid', listOper[listOper.findIndex(item=>axx = (item.icao == $(ele).val().toUpperCase()))]['craftid']);
    }
}
function CheckListAero(ele) {
    if (listAero.findIndex(item=>item.AE_REM == $(ele).val().toUpperCase()) == -1) {
        $(ele).val('');
        $(ele).css('border-color: red;');
    } else {
        $(ele).css('border-color: \'\'');
        $(ele).val(listAero[listAero.findIndex(item=>axx = (item.AE_REM == $(ele).val().toUpperCase()))].AE_CODE);
...
function reloadAutoComplete() {
    $('[data-AutoComplete="OPER"]').each(function () {
        ...
        $('#' + $ele.prop('id')).autocomplete({
            ...
            $('#' + $ele.prop('id')).val(data.name.split('-')[0].trim());
        }).on('blur', function (e, datum) {
            CheckListOper($('#' + $ele.prop('id')));
        });
    });
