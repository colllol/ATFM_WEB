...
{
    public partial class FinishedFlightsV3 : PageBaseCallBack
    {
        ...
        private clsDaylyFlightSearch GetObjectSearch()
        {
            ...
            obj.StartDate = DateTimeHelper.ConvertToDateTime(txtStartDate.Value);
            obj.FinishDate = DateTimeHelper.ConvertToDateTime(txtFinishDate.Value);
        ...
        private void LoadDataGrid(clsDaylyFlightSearch objSearch)
        {
            ...
            if (ddlSelect.Value == "0")
                dt = new FinishedFlightsDAL().GetTableBySearchDN(objSearch);
            else if (ddlSelect.Value == "1")
                dt = new FinishedFlightsDAL().GetTableBySearchDNNC(objSearch);
            else if (ddlSelect.Value == "2")
                dt = new FinishedFlightsDAL().GetTableBySearchDNATA(objSearch);
            else if (ddlSelect.Value == "3")
                dt = new FinishedFlightsDAL().GetTableBySearchDNBCS(objSearch);
            else if (ddlSelect.Value == "4")
                dt = new FinishedFlightsDAL().GetTableBySearchDNVIA(objSearch);
            else if (ddlSelect.Value == "5")
                dt = new FinishedFlightsDAL().GetTableBySearchDNSAMECAL(objSearch);
            else
                dt = new FinishedFlightsDAL().GetTableBySearchDNTRUNGCAL(objSearch);
