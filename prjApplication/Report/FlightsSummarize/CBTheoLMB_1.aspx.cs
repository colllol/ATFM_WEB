...
{
    public partial class CBTheoLMB_1 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request["FromDate"] != null && Request["FromDate"].ToString() != "" && Request["FromDate"].ToString() != String.Empty && Request["ToDate"] != null && Request["ToDate"].ToString() != "" && Request["ToDate"].ToString() != String.Empty)
            {
                ...
                if (!IsPostBack)
                {
                    //WriteInfoTime();
                    LoadData();
                    lblContent1.Text = "FromDate " + _dateFrom.Trim() + " ToDate " + _dateTo.Trim();
                    lblContent2.Text = LoadData();
                }
            }
        }
        ...
        protected string LoadData()
        {
            ...
            if (UltilFunc.checkDate(_dateFrom, _dateTo))
            {
                ...
                else
                {
                    ...
                    DataTable dt = _rpDAL.CBTheoLMB_1(_dateFrom.Trim(), _dateTo.Trim());
        ...
        private string GenDataTable(DataTable _dt)
        {
            ...
            if (_dt != null)
            {
                ...
                {
                    _mrow = _dt.Rows[j];
                    _a = _mrow["REAL_CRAFT_TYPE"].ToString();
                    if (_a != _b)
                    {
                        if (j == 0)
                        {
                            ...
                            _b = _mrow["REAL_CRAFT_TYPE"].ToString();
        ...
        {            
            ...
            this.CreatePDF(html, "CBTheoLMB_1_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/StyleReports.css"), System.Drawing.Printing.PaperKind.A4);
        }

        protected void btnExcel_Click(object sender, EventArgs e)
        {
            string html = this.RenderToHTML(divContent);
            this.CreateExcel(html, "CBTheoLMB_1_" + DateTime.Now.ToFileTime() + ".xls", Server.MapPath("~/Style/StyleReports.css"));
        }
        #endregion
    }
}