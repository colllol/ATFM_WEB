...
{
    public partial class TKTheoNVB_6 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request["FromDate"] != null && Request["FromDate"].ToString() != "" && Request["FromDate"].ToString() != String.Empty && Request["ToDate"] != null && Request["ToDate"].ToString() != "" && Request["ToDate"].ToString() != String.Empty)
            {
                ...
                if (!IsPostBack)
                {
                    WriteInfoTime();
                    LoadData();
                    lbldatekhoang.Text = "Fromdate" + _datekhoang.Trim() + "ToDate";
        ...
        protected string LoadData()
        {
            ...
            if (UltilFunc.checkDate(_dateFrom, _dateTo))
            {
                ...
                else
                {
                    ...
                    DataTable dt = _rpDAL.TKTheoNVB_6(_dateFrom.Trim(), _dateTo.Trim());
                ...
                for (int j = 0; j < _dt.Rows.Count; j++)
                {
                    ...
                    if (_a != _b)
                    {
                        if (j == 0)
                        {
                            ...
                            _html += "<td class=\"tg-yw4l\" style=\"text-align:center; \">" + _mrow["PURPOSE"].ToString() + "</td>";
                            _b = _mrow["PURPOSE"].ToString();
                        ...
                        {
                            _html += "<tr>";
                            _html += "<td class=\"tg -031e\" colspan=\"2\" style=\"text-align:center; \"><b>Sum Of Type" + " " + " " + _b.ToString() + "</b></td>";
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            this.CreatePDF(html, "TKTheoNVB_6_" + DateTime.Now.ToFileTime() + ".pdf", Server.MapPath("~/Style/StyleReports.css"), System.Drawing.Printing.PaperKind.A4);
        }

        protected void btnExcel_Click(object sender, EventArgs e)
        {
            string html = this.RenderToHTML(divContent);
            this.CreateExcel(html, "TKTheoNVB_6_" + DateTime.Now.ToFileTime() + ".xls", Server.MapPath("~/Style/StyleReports.css"));
        }
        #endregion
    }
}