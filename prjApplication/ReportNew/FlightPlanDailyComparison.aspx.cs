...
namespace prjApplication.ReportNew
{
    public partial class FlightPlanDailyComparison : ReportPageBase
    {
        ...
        [WebMethod]
        public static object GetComparison(string date1, string airport)
        {
            ...
            ComparisonDay day1 = LoadDay(first.Date, selectedAirport);
            ComparisonDay day2 = LoadDay(second, selectedAirport);
        ...
        private static ComparisonDay LoadDay(DateTime date, string airport)
        {
            const string sql = @"SELECT FLIGHTNBR, FROM_AIRP, TO_AIRP, ETD, ETA, FLIGHT_TYPE
                                   FROM T_DAY_FLIGHTS
            ...
            using (OracleCommand command = new OracleCommand(sql, connection))
            {
                ...
                {
                    while (reader.Read()) flights.Add(new FlightRow { Callsign = Text(reader["FLIGHTNBR"]), FromAirp = Text(reader["FROM_AIRP"]), ToAirp = Text(reader["TO_AIRP"]), Etd = Text(reader["ETD"]), Eta = Text(reader["ETA"]), FlightType = Text(reader["FLIGHT_TYPE"]).ToUpperInvariant() });
                }
            }
